...
public class SetFriendsRequestListParameter extends HttpServlet implements BaseGetParameter{
	...
	public Object register(HttpServletRequest req) {
		...
		//バリデーションチェック
		ErrorMessage error = FriendsListValidation.friendsListRegistValidation(allInfo);
		if(!(error.isEmpty())){
	...
	public Object moidfy(HttpServletRequest req) {
		...
		//バリデーションチェック
		ErrorMessage error = FriendsListValidation.friendsRequestListModifyValidation(allInfo);
		if(!(error.isEmpty())){
			HttpSession session = req.getSession();
			session.setAttribute("error", error);
		}
...
//		
//		//バリデーションチェック
//		ErrorMessage error = FriendsListValidation.friendsListModifyValidation(allInfo);
//		if(error.isEmpty()){
//			HttpSession session = req.getSession();
//			session.setAttribute("error", error);
//		}
