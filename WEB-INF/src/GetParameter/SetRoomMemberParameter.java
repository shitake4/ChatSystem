...
public class SetRoomMemberParameter extends HttpServlet implements
    BaseGetParameter, BasePermitParameter {
  ...
  public Object register(HttpServletRequest req) {
    ...
    if (addChatMembers == null || addChatMembers.equals("")) {
      ...
      for (int member_i = 0; member_i < addChatMembers.length; member_i++) {
        ...
        error = DoValidation.RoomMemberValidation
            .checkAccountIdValidation(addChatMemberList);
      }
    }
    if (!(error.isEmpty())) {
  ...
  public Object permit(HttpServletRequest req) {
    ...
    // バリデーション
    ErrorMessage error = RoomMemberValidation
        .RoomMemberValidation(permitAccount);
    if (!(error.isEmpty())) {
      HttpSession session = req.getSession();
      session.setAttribute("error", error);
    }
