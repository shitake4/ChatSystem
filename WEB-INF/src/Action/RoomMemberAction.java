...
public class RoomMemberAction implements BaseAction,BasePermitAction{
	...
	private static Logger logger = LoggerFactory.getLogger(RoomMemberAction.class.getName()); 
	...
	public String register(HttpServletRequest req, Object ob) {
		...
		for(int members_i = 0;members_i<addChatMemberList.size(); members_i++){
			try {
				int sqlResult = Model.UpdateDBManager.insertChatRoomMember(chatId, addChatMemberList.get(members_i).getAccountId());
				sumSqlResult +=sqlResult;
				logger.info("--------------------------sqlをループで回してます。sql結果：" + sqlResult + "--------------------------");
	...
	public String moidfy(HttpServletRequest req, Object ob) {
		...
		int sqlResult = 0;
		try {
			sqlResult = Model.UpdateDBManager.deleteChatRoomMember(chatId, memberAccountId);
