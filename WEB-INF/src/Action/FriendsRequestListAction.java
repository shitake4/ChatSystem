...
public class FriendsRequestListAction implements BaseAction {
  ...
  private static Logger logger = LoggerFactory
      .getLogger(FriendsRequestListAction.class.getName());
  ...
  public String register(HttpServletRequest req, Object ob) {
    ...
    if (accountId == friendsAccountId) {
      ...
      try {
        int sqlResult = UpdateDBManager.requestFriend(accountId,
            friendsAccountId);
        if (sqlResult != 0) {
  ...
  public String moidfy(HttpServletRequest req, Object ob) {
    ...
    int sqlResult = UpdateDBManager.agreeFriendRequest(accountId,
        friendsAccountId);
    if (sqlResult != 0) {
